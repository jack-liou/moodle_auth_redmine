 ...
 * Sample cron entry:
 * # 5 minutes past 4am
 * 5 4 * * * sudo -u www-data /usr/bin/php /var/www/moodle/auth/redmine/cli/sync_users.php
 *
 * Notes:
...
The auth_redmine plugin must be enabled and properly configured.
...
Example:
\$ sudo -u www-data /usr/bin/php auth/redmine/cli/sync_users.php
...
if (!is_enabled_auth('redmine')) {
    cli_error('auth_redmine plugin is disabled, synchronisation stopped', 2);
}

cli_problem('[AUTH DB] The sync users cron has been deprecated. Please use the scheduled task instead.');

// Abort execution of the CLI script if the \auth_redmine\task\sync_users is enabled.
$task = \core\task\manager::get_scheduled_task('auth_redmine\task\sync_users');
...
/** @var auth_plugin_redmine $dbauth */
